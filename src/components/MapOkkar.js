...
let options2 = {
  'q_learn': { matches: q_learn_matches, result: q_learn_result },
  'rql': { matches: rql_matches, result: rql_result },
  'fixed_h': { matches: fixed_h_matches, result: fixed_h_result },
  'var_h': {
...
const Map = () => {
  const chosenState = localStorage.getItem("option");
  console.log(chosenState);
  // dropdown menu - select matches and result array
  const [selectedOption, setSelectedOption] = useState(chosenState);
  const [resultArr, setResultArr] = useState(options2[chosenState].result);
  const [matchesArr, setMatchesArr] = useState(options2[chosenState].matches);
